...
def load_data():
    ...
    # Extract base indicator (strip quintile suffix) and quintile number
    def extract_base(name):
        for suffix in [": Q1 (lowest)", ": Q2", ": Q3", ": Q4", ": Q5 (highest)"]:
            if name.strip().endswith(suffix):
                return name.strip()[: -len(suffix)].strip()
        return name.strip()

    def extract_quintile(name):
        for q, label in [("Q1", "Q1 (Poorest)"), ("Q2", "Q2"), ("Q3", "Q3"),
                         ("Q4", "Q4"), ("Q5", "Q5 (Richest)")]:
            if f": {q}" in name:
                return label
        return "Unknown"
    ...
    df["Year Cols"] = [year_cols] * len(df)   # not stored; just for reference
